...
public class dbKudeatzailea {
	...
	public static boolean erabiltzaileaKonprobatu(String erabiltzailea, String pass) {

		try (Connection conn = getKonexioa(); Statement stmt = conn.createStatement()) {
			String sql = "SELECT erabiltzaile_izena, pasahitza\r\n"
					+ "FROM Erabiltzaileak\r\n"
					+ "WHERE erabiltzaile_izena = '" + erabiltzailea + "' && pasahitza = '" + pass + ";";
			stmt.executeUpdate(sql);
			return true;
		} catch (Exception e) {
			e.printStackTrace();
